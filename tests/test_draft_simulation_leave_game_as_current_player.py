...
def test_leave_game_as_current_player_same_players(db_session: Session, test_input, expected: int):
            ...
            while True:
                if test == 1:
                    ...
                    for future in games_leaving_futures.keys():
                        ...
                        formation = game_data[game]["formation"]
                        assert res is True
                        assert err == "new current player"
                        assert formation_name == formation
                        assert curr_pos == f"p{pos + 1}"
                        assert team_name not in teams
...
def test_leave_game_as_current_player_end_round_same_players(db_session: Session, test_input, expected: int):
    Session = sessionmaker(bind=db_session.bind)
    print("\n=====================\n", "test_start_round_same_players\n", sep="")
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        players_number_and_leave_status= {game:[len(valid_players_after_leaving[game]) - 1, False] for game in valid_games}
        for pos in range(POSITIONS):
            rand_teams_futures = {}
            for game, curr_player in picking_players.items():
                rand_teams_futures[executor.submit(thread_safe_rand_team, game, curr_player, Session)] = game 
            ...
            while True:
                games_leaving_futures = {}
                for game in valid_games:
                    if players_number_and_leave_status[game][0] != players_count_after_added_pos[game][0] or players_number_and_leave_status[game][1]:
                        continue
