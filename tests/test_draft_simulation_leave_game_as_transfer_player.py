...
@pytest.mark.parametrize("test_input, expected", [
    ({
        ...
        "players": {
                        ...
                        22:[111, 222, 333, 444, 555],
                        33:[111, 222, 333, 444, 555],
                        44:[111, 222, 333, 444, 555],
                        55:[111, 222, 333, 444, 555],
                        66:[111, 222, 333, 444, 555],
                        77:[111, 222, 333, 444, 555],
                        88:[111, 222, 333, 444, 555],
                        99:[111, 222, 333, 444, 555],
                    },
        "canceld": [22, 44],
...
def test_leave_game_before_transfers_same_players(db_session: Session, test_input, expected: int):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for game in test_input["games"]:
            ...
            if game in test_input["canceld"]:
                for player in test_input["players"][game]:
        ...
        num_players = {game:len(players_list) for game, players_list in test_input["players"].items()}
        ...
        for game in test_input["games"]:
            ...
            if len(game_data[game]["teams"]) != len(set(game_data[game]["teams"])):
                continue
            if len(game_data[game]["teams"]) != 11 + num_players[game]:
                continue
        ...
        for future in concurrent.futures.as_completed(start_game_futures.keys()):
            ...
            assert res is True
            assert err == ""
            assert num_players_ == num_players[game]
        ...
        for game in valid_games:
            ...
            state_set_games_futres[executor.submit(thread_safe_set_game_state ,game, test_input["players"][game][0], category, teams, formation, Session)] = game
        ...
        for future in concurrent.futures.as_completed(state_set_games_futres.keys()):
            ...
            assert curr_player_id_ in test_input["players"][game] 
        ...
        for game in valid_games:
            new_list = []
            for player_id in test_input["players"][game]:
                new_list.append(tuple([player_id, {f"p{x}":"" for x in range(1, 12)}]))

            players_teams[game] = new_list

        valid_players_after_leaving = {game:players_list for game, players_list in test_input["players"].items()}
        ...
        for pos in range(POSITIONS):
            ...
            added_pos_futures = {}
            players_count_after_added_pos = {game:[0, False] for game in valid_games}
            while True:
                for game, curr_player in curr_players.items():
                    if players_count_after_added_pos[game][1]:
                        continue
                ...
                for future in concurrent.futures.as_completed(added_pos_futures.values()):
                    ...
                    print("\n==========================\n", "i: ", players_count_after_added_pos[game][0], "num_players: ", num_players[game] - 1,  game, res, err, curr_player_id, curr_pos_, formation_, "\n==========================\n")

                    if players_count_after_added_pos[game][0] == len(valid_players_after_leaving[game]) - 1:
                        formation = game_data[game]["formation"]
                        assert res is True
                        ...
                        players_count_after_added_pos[game][1] = True
                    else:
                        formation = game_data[game]["formation"]
                        assert res is True
                        assert err == "same_pos"
                        assert (curr_player_id in valid_players_after_leaving[game]) is True
                        assert formation_ == formation
                        assert curr_pos_ == f"p{pos + 1}"
                    curr_players[game] = curr_player_id
                    players_count_after_added_pos[game][0] += 1

                if all(status[1] for status in players_count_after_added_pos.values()):
                    break
        ...
        players_count_after_transfer = {game:[0, False] for game in valid_games}
        test = 1
        while True:
            if test == 1:
                games_leaving_futures = {}
                for game, current_player_id in curr_players.items():
                    for team in players_teams[game]:
                        if team[0] == current_player_id:
                            players_teams[game].remove(team)
                ...
                for future in games_leaving_futures.keys():
                    ...
                    formation = game_data[game]["formation"]
                    assert res is True
                    assert err == "new transfer player"
                    assert formation_name == formation
                    assert curr_pos == "p11"
                    assert next_player_id in valid_players_after_leaving[game]
                    assert non_picked_teams == teams

                    print("\n==========================\n", "player left: ", picking_players[game], "new player: ", next_player_id, "\n==========================\n")

                    valid_players_after_leaving[game].remove(picking_players[game])
                    picking_players[game] = next_player_id
                    curr_players[game] = next_player_id
            ...
            for game, current_player in curr_players.items():
                if players_count_after_transfer[game][1]:
                    continue
            ...
            games_with_round_ended = {executor.submit(thread_safe_end_round, game, Session):game for game in valid_games if not players_count_after_transfer[game][1]}

            for future in concurrent.futures.as_completed(games_with_round_ended.keys()):
                game = games_with_round_ended[future]
                res, err, curr_player_id, formation_, teams = future.result()

                print("\n==========================\n", players_count_after_transfer[game][0], len(valid_players_after_leaving[game]),  game, res, err, curr_player_id, formation_, "\n==========================\n")

                if players_count_after_transfer[game][0] == len(valid_players_after_leaving[game]) - 1:
                    formation = game_data[game]["formation"]
                    assert res is True
                    assert err == "end_game"
                    assert (curr_player_id in valid_players_after_leaving[game]) is True
                    assert formation_ == formation
                    players_count_after_transfer[game][1] = True
                else:
                    formation = game_data[game]["formation"]
                    assert res is True
                ...
                players_count_after_transfer[game][0] += 1
            if all(status[1] for status in players_count_after_transfer.values()):
                break
        ...
        game_vote_count = {}
        for game in valid_games:
            players_options[game] = {}
            players_vote_list = [[], []]
            for option, id in enumerate(valid_players_after_leaving[game]):
                players_vote_list[0].append([id])
                players_vote_list[1].append(f"player_{game}_{id}")

                players_options[game][option] = id
            ...
            game_vote_count[game] = {player_id[0]:0 for player_id in players_vote_list[0]} 
        ...
        for i, (game, future_list) in enumerate(games_add_votes_futures.items()):
            ...
            assert games_players_voting_count[game] == len(valid_players_after_leaving[game])
...
def test_leave_game_before_transfers_end_game_same_players(db_session: Session, test_input, expected: int):
    Session = sessionmaker(bind=db_session.bind)
    print("\n=====================\n", "test_start_round_same_players\n", sep="")
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        players_count_after_transfer = {game:[0, False] for game in valid_games}
        games_player_count_and_ended= {game:[len(valid_players_after_leaving[game]), False] for game in valid_games}
        count  = 1
        while True:
            games_leaving_futures = {}
            for game in valid_games:
                if games_player_count_and_ended[game][0] != count or games_player_count_and_ended[game][1]:
                    continue

                current_player_id = curr_players[game]
                for team in players_teams[game]:
                    if team[0] == current_player_id:
                        players_teams[game].remove(team)
