...
def test_leave_game_before_transfers_same_players(db_session: Session, test_input, expected: int):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for game, picking_player_id in picking_players.items():
            for team in players_teams[game]:
                if team[0] == picking_player_id:
                    players_teams[game].remove(team)

            games_leaving_futures[executor.submit(thread_safe_leave_game, game, picking_player_id, Session)] = game 
